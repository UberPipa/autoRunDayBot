...
async def reference(call: types.CallbackQuery, state: FSMContext) -> None:
    ...
    await bot.edit_message_text(
        chat_id=call.from_user.id,
        message_id=message_id,
        text='- Бот полностью перенесён на инлайн кнопки\n'
             '- Появился учёт рабочего времени и рекомендуемое время завершения\n'
             '- Появилась возможность ставить рабочий день на паузу\n'
             '\n'
             '<b>Меню настроек:</b>\n'
             '- Авто завершение дня:\n'
             'Когда включён - принудительно завершает день в 21:00, если он открыт.\n'
             '- Сменить логопас:\n'
             'При нажатии удаляет текущий логин и пароль, бот просит ввести его заново.\n',
